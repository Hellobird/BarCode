...
package io.hellobird.barcode.camera;
...
import io.hellobird.barcode.camera.open.OpenCamera;
import io.hellobird.barcode.camera.open.OpenCameraInterface;
...
public final class CameraManager {
     ...
     * 扫描框比率
     */
    private float mFrameRation;

    /**
     * 是否同步扫描范围，
     * 为true时，只能当识别码在UI扫描框范围内时才能识别
     * 为false时，超出UI扫描框一样可以识别
     */
    private boolean mSyncScanFrame;

    public CameraManager(Context context) {
        this(context, FRAME_RATIO,true);
    }

    public CameraManager(Context context, float ratio, boolean syncScanFrame) {
        this.context = context;
        this.configManager = new CameraConfigurationManager(context);
        previewCallback = new PreviewCallback(configManager);
        mFrameRation = ratio;
        mSyncScanFrame = syncScanFrame;
    }
    ...
    public synchronized Rect getFramingRect() {
        if (framingRect == null) {
            ...
            // 原本的扫描框计算规则，长宽分别根据5/8来计算
//            int width = findDesiredDimensionInRange(screenResolution.x, MIN_FRAME_WIDTH, MAX_FRAME_WIDTH);
//            int height = findDesiredDimensionInRange(screenResolution.y, MIN_FRAME_HEIGHT, MAX_FRAME_HEIGHT);
            // 改为根据外部比例传入来计算
            int width = (int) (screenResolution.x * mFrameRation);
            int height = (int) (screenResolution.y * mFrameRation);
            // 两者取其小
            if (width > height) {
                width = height;
            } else {
    ...
    public synchronized Rect getFramingRectInPreview() {
        if (framingRectInPreview == null) {
            ...
            if (!isSameDirection()) {
                // 交换相机宽高
                cameraX = cameraResolution.y;
                cameraY = cameraResolution.x;
            }
            if (mSyncScanFrame) {
